...
plot!(p12, dat_trials[k].time, out_sim.lwr, label="Observed Data", alpha=0.25, linewidth=1)
plot!(p12, dat_trials[k].time, out_sim.upr, label="Observed Data", alpha=0.25, linewidth=1)
...
# Gamma Parameter Estimation Plot
p13 = plot()
scatter!(1:n, theta_true[k][:gamma], label="Gamma True", xlabel="Position", ylabel="Value", color=:red, marker=:circle)
scatter!(1:n, results[:chain][n_iter][:gamma], label="Mean of Sampled Gammas", color=:blue, marker=:circle)
display(p13)

# Beta Parameter Estimation Plot
p15 = plot()
scatter!(1:4, theta_true[k][:beta], label="Beta True", xlabel="Position", ylabel="Value", color=:red, marker=:circle)
scatter!(1:4, results[:chain][n_iter][:beta], label="Betas Estimate", color=:blue, marker=:circle)
display(p15)
